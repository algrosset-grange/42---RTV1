...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/11/23 14:36:52 by agrosset          #+#    #+#             */
/*   Updated: 2017/11/23 14:36:53 by agrosset         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rtv1.h"

static int	ft_translation_light_p2(t_checkjson *check, t_scene *scene)
{
	t_light	*current;
...
int			ft_translation_light(t_checkjson *check, t_scene *scene)
{
	if (check->annonce_vector == NOT_DONE)
	{
			...
			&check->annonce_vector) == NO_ERROR)
		{
			...
			check->translation == DONE)
				if (ft_check_multi_light(check, scene) == ERROR)
					return (ERROR);
	...
	if (ft_translation_light_p2(check, scene) == ERROR)
		return (ERROR);
...
static int	ft_origin_light_p2(t_checkjson *check, t_scene *scene)
{
	t_light	*current;
...
int			ft_origin_light(t_checkjson *check, t_scene *scene)
{
	...
	if (ft_origin_light_p2(check, scene) == ERROR)
		return (ERROR);
	if (check->nb_curly_bracket < 4)
	{
		ft_re_init_vect(check);
