...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/11/23 14:45:08 by agrosset          #+#    #+#             */
/*   Updated: 2017/11/23 14:45:09 by agrosset         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rtv1.h"

static void	ft_rotation_cylinder_p3(t_scene *scene)
{
	t_cylinder	*current;
...
static int	ft_rotation_cylinder_p2(t_checkjson *check, t_scene *scene)
{
	t_cylinder	*current;
...
int			ft_rotation_cylinder(t_checkjson *check, t_scene *scene)
{
	...
	if (ft_rotation_cylinder_p2(check, scene) == ERROR)
		return (ERROR);
	if (check->nb_curly_bracket < 4)
	{
		ft_re_init_vect(check);
		ft_rotation_cylinder_p3(scene);
		check->rotation = DONE;
	}
	return (NO_ERROR);
}

static int	ft_translation_cylinder_p2(t_checkjson *check, t_scene *scene)
{
	t_cylinder	*current;
...
int			ft_translation_cylinder(t_checkjson *check, t_scene *scene)
{
	...
	if (ft_translation_cylinder_p2(check, scene) == ERROR)
		return (ERROR);
	if (check->nb_curly_bracket < 4)
	{
		ft_re_init_vect(check);
